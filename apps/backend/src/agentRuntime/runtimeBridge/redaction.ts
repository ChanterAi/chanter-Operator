// ── CHANTER Operator P1A: Runtime Bridge — Redaction Mirror ──
//
// Structural mirror of chanter-agent-runtime's `src/redaction.ts` (see
// contract.ts for why this is mirrored rather than imported). Applied at the
// bridge's write boundary (tasks.ts) and again, defensively, at its export
// boundary (evidence.ts) so secrets accidentally passed into the bridge —
// API keys, bearer tokens, private keys, password/secret/token fields — never
// survive into logs, evidence bundles, or readiness reports.
//
...
//
// DELIBERATE HARDENING BEYOND THE UPSTREAM SOURCE: while writing this bridge's
// test suite, an end-to-end secret-leakage test found that the upstream
// chanter-agent-runtime `tasks.ts`/`evidence.ts` (as of commit bd1f310) never
// redacts `RuntimeTask.objective`, `RuntimeResult.summary`, or
// `RuntimeValidationResult.summary` at either the write or export boundary —
// only `inputs`, evidence `detail`/`source`, validation check `message`,
// result `output`, and recommendation `reason` are covered. Since this loop
// cannot modify chanter-agent-runtime, tasks.ts and evidence.ts in this
// directory additionally redact those three fields at both boundaries.
// This is a one-line deviation from a literal mirror, flagged for upstreaming
// in the next chanter-agent-runtime hardening pass.
