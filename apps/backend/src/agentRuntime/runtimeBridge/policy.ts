// ── CHANTER Operator P1A: Runtime Bridge — Action Policy Mirror ──
//
// Structural mirror of chanter-agent-runtime's `src/policy.ts` (see
// contract.ts for the "why mirrored, not imported" rationale).
//
// A separate, additive gate from the task lifecycle in contract.ts: where
...
import type { RuntimeExecutionPolicy, RuntimeTask } from "./contract.js";
import { isTerminalStatus } from "./contract.js";
 ...
 * encode "high/critical risk requires approval" via contract.ts, and the
