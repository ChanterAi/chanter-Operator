...
import { assertTransitionAllowed, isTerminalStatus, requiresApprovalBeforeExecution } from "./contract.js";
...
export function createTask(input: CreateTaskInput): RuntimeTask {
  ...
  const task: RuntimeTask = {
    ...
    // Redacted here (deliberately hardened beyond the upstream chanter-agent-runtime
    // source, which leaves `objective` unredacted) — see redaction.ts doc comment.
...
export function approveTask(task: RuntimeTask, approver?: string, note?: string): RuntimeTask {
  const message = approver ? `Approved by ${approver}${note ? `: ${note}` : ""}` : (note ?? `Task ${task.id} approved.`);
 ...
 * transition table (see contract.ts), not by a separate check here.
 */
...
function buildEvidence(input: RuntimeEvidenceInput): RuntimeEvidence {
  return {
    ...
    // JSON without a key-count mismatch — see assertJsonSafe in evidence.ts.
...
function buildValidationResult(input: RuntimeValidationInput): RuntimeValidationResult {
  ...
  // Redacted here (deliberately hardened beyond the upstream chanter-agent-runtime
  // source, which leaves `summary` unredacted) — see redaction.ts doc comment.
  return { passed, checks, summary: redactText(summary), validatedAt: nowIso() };
}
...
export function attachRecommendation(task: RuntimeTask, input: RuntimeRecommendationInput): RuntimeTask {
  ...
  return mutateTask(task, "RECOMMENDATION_ATTACHED", `Recommendation: ${recommendation.action} — ${recommendation.reason}`, (draft) => {
