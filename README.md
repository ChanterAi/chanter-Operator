...
## SafeCommit closeout approval authority

Operator owns one standalone, closed-world approval action: `safecommit.closeout.execute`. It persists only immutable request identity plus the exact external `chanter.safecommit.closeout.v1` plan hash; SafeCommit remains the plan compiler, Git executor, recovery-journal owner, and closeout-evidence owner. Operator never receives Git commands or normalized plan payloads and never stages, commits, or pushes.

- `POST /api/safecommit-closeouts` and `GET /api/safecommit-closeouts/:requestId` require `OPERATOR_MISSION_SUBMIT_TOKEN`; submission/read cannot approve or execute.
- `POST .../approve` and `POST .../revoke` require the independent `OPERATOR_CONTROL_TOKEN`. Approval binds the exact plan hash plus founder actor, the fixed reviewed basis, optional note, and Operator timestamp; approval persists no execution side effect.
- `POST .../claim`, `POST .../invalidate`, and `POST .../complete` require the distinct `OPERATOR_SAFECOMMIT_EXECUTOR_TOKEN`, which must never be shared with MCP or browser code. It can only atomically claim an already-approved hash, fail closed when repository truth changes, or attach an opaque evidence reference and SHA-256 digest.
- The append-only SQLite event record and guarded lifecycle are `approval_required -> approved -> execution_claimed -> completed | failed_terminal`; pre-claim approval may be revoked, while approved or claimed truth may be invalidated. Recoverable execution remains claimed and is owned by SafeCommit's journal.
...
All configured write capabilities — mission submit, mission control, SafeCommit executor, ledger ingest, and the AutoPoster Runtime service token — must use distinct values. Any protected route whose configured capability collides with another capability fails closed with `CAPABILITY_TOKEN_CONFIGURATION_INVALID`.
